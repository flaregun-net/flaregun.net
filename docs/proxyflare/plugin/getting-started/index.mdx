...
<CustomTOCList>
  <CustomTOCListSection>
    <CustomTOCListHead>Articles</CustomTOCListHead>
    <CustomTOCListContent>
      <VersionedLink to="/proxyflare/plugin/configure/services">
        Proxy to a web service
      </VersionedLink>
    </CustomTOCListContent>
    <CustomTOCListContent>
      <VersionedLink to="/proxyflare/plugin/configure/websites">
        Proxy to a web site
      </VersionedLink>
    </CustomTOCListContent>
    <CustomTOCListContent>
      <VersionedLink to="/proxyflare/plugin/configure/redirects">
...
Proxyflare moves traffic around your domain using `Route`s. A `Route` sends traffic to another service on your domain or elsewhere on the internet by generating a `ProxiedRequest` and handling the proxied response.
...
Proxyflare can send incoming traffic on your domain to anywhere else on the internet. Let's say you wanted to send traffic on `https://proxyflare.works/api` to a service at `https://my-api.com`.
...
In `apiRoute`, Proxyflare sends traffic from `proxyflare.works/api/*` to `my-api.com`.

Notice the wildcard pathname in `Route["from.pattern"]`. This means that traffic on subpaths of `proxyflare.works/api/` such as `proxyflare.works/api/users/1` goes to `my-api.com/users/1`. If `Route["from.pattern"]` didn't include a wildcard, subpaths of `Route["from.pattern"]` wouldn't match `apiRoute`.

:::info Note

<i>https:</i> is assumed unless <i>http:</i> is specified in <code>
  Route["to.url"]
</code>

:::

#### On a desired port on a domain

The following `Route` sends traffic on `proxyflare.works/api/*` to the service listening on `flaregun.net:8787`.
...
For example, traffic on `https://proxyflare.works/example/page` gets sent to `https://example.com/page`.

:::caution Is your service available?

Because Proxyflare uses the internet to move traffic, any service it proxies must also be accessible on the internet.
...
For example, an AWS service deployed at `my-api.ec2.amazonaws.com:8787` would have to create a Network Rule that opens port 8787 before using Proxyflare. Once `my-api.ec2.amazonaws.com:8787` is accessible from a browser, Proxyflare can connect it to your domain.

:::

### Proxy to another website

Imagine we have separate documentation and blogging sites that we want to mount on `proxyflare.works/docs` and `proxyflare.works/blog`.

Proxyflare can proxy content rich websites with a little additional setup. You'll need to be aware of the following concepts described below:
...
Most modern website engines such as Wordpress and Docusaurus allow you to set the base URL. The base URL is the root path after the host. For example, `/docs/` is the base URL of the Docusaurus-powered documentation website proxied on https://proxyflare.works/docs.

In order for your website to generate accurate links and route correctly, its base URL must match your Proxyflare `Route["from.pattern"]`. For our own documentation site, we set:

- In _docusaurus.config.js_, `baseUrl = /blog/`
- In our Proxyflare route, `Route["from.pattern"]: proxyflare.works/blog/*`

To learn how to set the base URL for your site, please refer to the website engine documentation.

- Set `baseURL` [in Docusaurus](https://docusaurus.io/docs/api/docusaurus-config#baseUrl)
- Set `siteurl` [in WordPress](https://wordpress.org/support/article/changing-the-site-url/)
- Set up custom domain [in Redocly](https://redocly.com/docs/developer-portal/settings/custom-domain/)
- Set `django-base-url` [in Django](https://pypi.org/project/django-base-url/)

#### Route["to.website.mode"]: true | "spa"

This option determines how Proxyflare routes website requests. If your website engine outputs statically rendered content, such as WordPress or Django, no configuration is needed (the default value `true` will be used).

For websites and website engines that run on modern single page app frameworks like on React, Angular, and Vue, set `to.website.mode="spa"` to leverage its client-side router.
...
But how did we know to add these particular patterns in `to.website.resources`? The quickest way is to open your browser's `Developer Tools > Network` to identify failing requests for website resources like images, scripts, and stylesheets.
...
Inspect the `Network` tab on https://proxyflare.works/docs to see if you can identify why we included the three entries `docsRoute["to.website.resources"]` above.
