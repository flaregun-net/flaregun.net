...
(_assuming this variable is being used to indicate the author's ID_):
...
`agent` from the `authors` table to another table called `agent` (_an
author can have an agent_) and we want to allow users with the role
...
This permission rule reads as "_if the author's agent's_ `id` _is the
same as the requesting user's_ `id` _, allow access to the author's
details_."
...
- The above example would have worked even if the relationship were an
  ...
  array relationship would have read "_if any of this author's
  agents'_ `id` _is the same as the requesting user's_ `id` _, allow
...
This permission rule reads as "_if there exists a row in the table_
`users` _whose_ `id` _is the same as the requesting user's_ `id` _and
has the_ `allow_article_create` _column set to true, allow access to
insert articles_."
...
accessible rows (_based on the rule_:
