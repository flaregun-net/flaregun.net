...
2. In the _Ports_ section, change the export port from `80` to `8080`, which is the port the `hasura/graphql-engine` Docker image app is
...
3. In the _Environment variables_ section, configure the environment variables required to properly run the Hasura GraphQL engine.

   - flaregun_GRAPHQL_DATABASE_URL: The environment variable containing the PostgreSQL URL, i.e. `postgres://<user>:<password>@<host>:<port>/<database>`.
   ...
   - flaregun_GRAPHQL_ENABLE_CONSOLE: Set to `true`. This will expose and allow you to access the Hasura console.
   - flaregun_GRAPHQL_ADMIN_SECRET: The secret to access the Hasura Graphql admin. As for the `HASURA_GRAPHQL_DATABASE_URL`, we strongly recommend using
