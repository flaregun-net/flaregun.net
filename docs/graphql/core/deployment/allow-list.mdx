...
The **Allow-list** is a list of safe operations (_GraphQL queries,
mutations or subscriptions_) that is stored by the GraphQL engine in its
...
_(available after version v1.0.0-beta.1)_.
...
running the GraphQL engine with the `--enable-allowlist` flag (_default
value is_ `false`). See [reference docs](/graphql/core/deployment/graphql-engine-flags/reference.mdx#command-flags).
...
- **In development instances**: During development or in dev
  instances, disable allow-list (_default setting_) to allow complete
  access to the GraphQL schema. Add/remove operations in the
  allow-list and then export the metadata for version-control (_so you
  can apply it to other instances_).
