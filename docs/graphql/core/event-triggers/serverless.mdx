---
sidebar_label: Using serverless functions
sidebar_position: 5
...
For this we [setup an event trigger](/graphql/core/event-triggers/create-trigger.mdx) on `UPDATE` to the
`notes` table which calls an AWS Lambda function. The AWS Lambda
function itself uses a GraphQL mutation to insert a new row into the
`note_revision` table. As the [event trigger payload](/graphql/core/event-triggers/payload.mdx) in case of updates gives us both the old and the new data, we can store
...
exports.handler = (event, context, callback) => {
  ...
  const qv = {
    noteId: request.event.data.old.id,
    data: request.event.data.old.note,
  }
