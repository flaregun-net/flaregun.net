...
- In case of `UPDATE`, the events are delivered only if new data is
  ...
  which cannot be compared using `<>` operator, then internal binary
  representation of rows by Postgres is compared.
- [Table computed fields](/graphql/core/databases/postgres/schema/computed-fields.mdx) are not included in the
