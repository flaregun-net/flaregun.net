---
sidebar_label: Setting up migrations
sidebar_position: 1
...
tables/views and schema modifying SQL statements, as SQL files. These
...
To use migrations effectively, the console served by the server (which is served at `/console`) should be disabled and all changes must go through
...
Follow the instructions in [Installing the Hasura CLI](/graphql/core/hasura-cli/install-hasura-cli.mdx).

## Step 2: Set up a project directory {#migrations-project-init}

For the endpoint referred here, let's say you've deployed the GraphQL
engine on Hasura Cloud, then this endpoint is:
`https://my-graphql.hasura.app`. In case you've deployed Hasura using
Docker, the URL might be `http://xx.xx.xx.xx:8080`. In any case, the
endpoint should **not** contain the `v1/graphql` API path. It should
...
`config.yaml` file, a `migrations` directory and a `metadata` directory.
...
## Step 3: Initialize the migrations and metadata as per your current state {#migrations-initialize}

If you have already set up your database and GraphQL API, you need to
initialize your database migrations and Hasura metadata with the current
...
This command will create a new directory named `<timestamp>_init` inside
the `migrations/<database-name>` directory. In the newly created
directory, there's a file named `up.sql`. This file will contain the
...
the format of migration files, check out the [Migration file format reference](/graphql/core/migrations/reference/migration-file-format.mdx).
...
If you'd like to read more about the format of metadata files, check out
the [Metadata format reference](/graphql/core/migrations/reference/metadata-format.mdx).
...
The version control set up should typically be done right after [Step 2](#migrations-project-init)
...
`http://my-graphql.hasura.app/console` you should use the console served by the CLI by running:
...
`migrations/` directory and the metadata is exported in the `metadata/`
directory of your project.

Let's create the following table
`address (id uuid, street text, zip text, city text, country text, author_id uuid)`
and then create a foreign-key to the `author` table via the
`author_id -> id` columns.

In the `migrations/<database-name>` directory, we can find new
directories called `<timestamp>_create_table_public_address` and
`<timestamp>_set_fk_public_address_author_id` containing an `up.sql` and
a `down.sql` migration files for the changes we made.
...
exported into the `metadata` directory.
...
As you keep using the console via the CLI to make changes to the schema,
new migration files will keep getting generated and the metadata files
...
feature. To improve maintainability of the migration files and to ensure
you can go back to a particular version of the metadata, it is
recommended that you squash your migration files and commit the project
status in version control whenever you reach a logical checkpoint in
your feature development.

The following command will squash all migration files from the given
migration to the latest migration into a single migration file.
...
Apply all migrations present in the `migrations/` directory and the
metadata present in the `metadata/` directory on a new instance at
...
In case you need an automated way of applying the migrations and
metadata, take a look at the [cli-migrations](/graphql/core/migrations/advanced/auto-apply-migrations.mdx)
Docker image, which can start the GraphQL engine after automatically
applying the migrations and metadata which are mounted onto directories.

As we are applying metadata before migrations, your metadata might be in
an **inconsistent state** after the initial `metadata apply` till the
`metadata reload` step as some database objects referred to in metadata
might not be available till the migrations are applied.

If you now open the console of the new instance, you can see that the
three tables have been created and are tracked:

<Thumbnail
  src="/img/graphql/core/migrations/tracked-tables.png"
...
`migrations` directory along with its name, source status and database
status.
...
Such a migration status indicates that there are 2 migration versions in
the local directory and both of them are applied on the database.
...
the database yet. Executing `hasura migrate apply` will resolve this.
