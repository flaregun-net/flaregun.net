...
- You can pass an `on_conflict` argument to convert the mutation to an
  [upsert mutation](/graphql/core/databases/postgres/mutations/upsert.mdx).
...
See the [insert mutation API reference](/graphql/core/api-reference/graphql-api/mutation.mdx#insert-upsert-syntax) for the
full specifications.
...
[insertion_order](/graphql/core/api-reference/syntax-defs.mdx#objrelusingmanualmapping) option while creating a
manual relationship. This is necessary to ensure
[nested inserts](/graphql/core/guides/data-modelling/one-to-one.mdx#one-to-one-insert) in one-to-one relationships are
...
[many-to-many relationship](/graphql/core/guides/data-modelling/many-to-many.mdx) with the `tags`
...
2.  The array relationship objects (from the perspective of `article`)
    ...
    2.  The parent object (from the perspective of `article_tags`) is
        ...
        `tag_id` in the data for the <em>article_tags</em> relationship.
...
[nested upsert caveats](/graphql/core/databases/postgres/mutations/upsert.mdx#pg-nested-upsert-caveats)).
...
[default value configuration](/graphql/core/databases/postgres/schema/default-values/postgres-defaults.mdx) i.e. via Postgres
