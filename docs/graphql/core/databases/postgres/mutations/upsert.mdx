...
To convert an [insert mutation](/graphql/core/databases/postgres/mutations/insert.mdx) into an upsert, you need to
use the `on_conflict` argument to specify:
...
values for the `constraint` field _(try autocompleting in GraphiQL)_.
...
[here](/graphql/core/databases/postgres/mutations/insert.mdx#pg-nested-inserts).
