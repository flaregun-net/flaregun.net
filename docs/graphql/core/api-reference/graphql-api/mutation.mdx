...
The conflict clause is used to convert an _insert_ mutation to an
_upsert_ mutation. _Upsert_ respects the table's _update_ permissions
before editing an existing row in case of a conflict. Hence the conflict
clause is permitted only if a table has _update_ permissions defined.
...
key columns in _update_ mutations.
